...
public class ZfbRechargeCheckerRunnable extends AbstractRechargeCheckerRunnable {
	...
	protected void process(IPayConfig payConfig,
		...
		for (UserTransaction userTransaction : userTransactionList) {
			...
			try {
				...
				logger.error("充值查询接口异常" + e.getMessage());
				continue;
			}
			//解析
			...
			try {
				if ("F".equals(is_success)) {
					...
					if (Constants.zfbError.containsKey(error)) {
						...
						logger.error("支付宝查询订单{}充值异常{}", new Object[] {
								userTransaction.getId(),orderStr});
					}
				...
				logger.error("支付宝充值订单:{},处理结果异常",userTransaction.getId(), e);
			}
          }
		}
	}
