 ...
 *
 */
public class XfWapRechargeCheckerRunnable extends AbstractRechargeCheckerRunnable {
				...
				if(StringUtils.isNotBlank(resultString)){
					...
					if("00".equals(respcode)&&"00".equals(origRespCode)){//成功或已成功
						...
						if(Constants.elinPckFailedError.containsKey(respcode)){
							...
							if(leftTime>60){
								processResult(userTransaction.getId(),queryId,tradeAmt,false,respMsg);
							}
						}else if("05".equals(origRespCode)){
							long leftTime=(new Date().getTime()-userTransaction.getCreateTime().getTime())/60000;
							if(leftTime>60){
								userTransaction.setStatus(PayStatus.PAY_FAILED.getValue());
								processResult(userTransaction.getId(),queryId,tradeAmt,false,"订单长时间未支付失败");
							}
						}else if("77".equals(origRespCode)||"64".equals(origRespCode)){//银行卡未开通认证支付余额不足
							userTransaction.setStatus(PayStatus.PAY_FAILED.getValue());
							processResult(userTransaction.getId(),queryId,tradeAmt,false,"订单银行卡未开通认证支付失败");
						}else{
							long leftTime=(new Date().getTime()-userTransaction.getCreateTime().getTime())/60000;
							if(leftTime>60){
								userTransaction.setStatus(PayStatus.PAY_FAILED.getValue());
								processResult(userTransaction.getId(),queryId,tradeAmt,false,"充值异常");
							}
						}
					}
				}
			} catch (Exception e) {
				logger.error("银联手机控件充值查询出错id={}",userTransaction.getId(),e);
			}
