...
public class ZfbWapRechargeCheckerRunnable extends AbstractRechargeCheckerRunnable {
	...
	protected void process(IPayConfig payConfig,
		...
		for (UserTransaction userTransaction : userTransactionList) {
			...
			try {
				...
				logger.error("支付宝wap充值通知解析格式异常", e);
				continue;
			}
			//结果处理
			try {
				if ("F".equals(is_success)) {
					...
					if (Constants.zfbError.containsKey(error)) {
						...
						logger.error("支付宝wap查询订单{}充值异常{}", new Object[] {
								userTransaction.getId(),
								Constants.zfbError.get(error) });
					}
					...
					if (userTrans != null) {
						if(PayStatus.PAY_FAILED.getValue()==userTrans.getStatus()){
							processResult(userTrans.getId(),userTrans.getTradeNo(),String.valueOf(userTransaction.getAmount()),false,userTrans.getDescription());
						}else if(PayStatus.PAY_SUCCESS.getValue()==userTrans.getStatus()){
							processResult(userTrans.getId(),userTrans.getTradeNo(),String.valueOf(userTransaction.getAmount()),true,"");
						}
						
					}
				}
			} catch (Exception e) {
				logger.error("支付宝wap充值订单{}处理异常",userTransaction.getId(),e);
			}
          }
		}
	}
	...
	public UserTransaction getChargeResult(UserTransaction userTransaction,String orderStr) throws DocumentException {
				...
				if (Constants.TRADE_SUCCESS.equals(trade_status)||"TRADE_FINISHED".equals(trade_status)) {
