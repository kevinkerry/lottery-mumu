	...
	private void process(JSONObject jsonObject){
		try{
			...
			if(jclqRace==null){
				...
				if(jclqRace.getStatus() != RaceStatus.PAUSE.value && jclqRace.getStatus() != RaceStatus.CANCEL.value){

					if(jclqRace.getEndSaleTime().getTime()!=endTime.getTime()){
						logger.error("竞彩篮球:{},比赛时间发生改变",matchNum);
						jclqRace.setEndSaleTime(endTime);
						jclqRace.setOfficialDate(officialDate);
					}
					jclqRace.setStaticHandicap(staticHandicap);      
					jclqRace.setStaticPresetScore(staticPresetScore);
