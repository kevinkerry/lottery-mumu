...
public class Jclqdxf extends AbstractJingcaiLot{
			...
			for(JingcaiCodeItem item:items) {
				...
				if(jingcaiResult.isCancel()) {
					...
					if(danguanType.equals(type)) {
						/**
						String prizecode = jingcaiResult.getResult_dynamic();
						if(item.getCodeItem().containsKey(prizecode)) {
							amt = amt.multiply(new BigDecimal(jingcaiResult.getPrize_dynamic()).divide(new BigDecimal(2)));
						}else {
							amt = BigDecimal.ZERO;
							break;
						}*/
	...
	protected JingcaiResult getJingcaiResult(JclqRace race, JingcaiCodeItem item) {
		...
		if(item.getType().equals(danguanType)) {
			if(sum>Double.parseDouble(race.getDynamicPresetScore().replace("+", ""))) {
				result.setResult_dynamic("1");
			}else {
				result.setResult_dynamic("2");
			}
		}else {
		...
		result.setPrize_dynamic(race.getPrizeDxf());
		*/
