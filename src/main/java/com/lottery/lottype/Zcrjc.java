...
import com.lottery.lottype.zc.renjiu.Zucairenjiu01;
import com.lottery.lottype.zc.renjiu.Zucairenjiu02;
import com.lottery.lottype.zc.renjiu.Zucairenjiu03;

@Component("4002")
public class Zcrjc extends AbstractLot{

	private Logger logger = LoggerFactory.getLogger(Zcrjc.class);
	
	Map<String, LotPlayType> map = new HashMap<String, LotPlayType>();
	{
		map.put("400201", new Zucairenjiu01());
		map.put("400202", new Zucairenjiu02());
		map.put("400203", new Zucairenjiu03());
	}
	...
	public List<SplitedLot> split(String betcode, int lotmulti, long amt,
		...
		long totalAmt = 0;
		if(isZcSingleUpload(betcode, String.valueOf(PlayType.zc_rjc_dan.value))) {
	...
	public Map<String, Long> getAwardLevels(int lotterytype, String phase) {
		...
		String[] infos1 = lotteryPhase.getPrizeDetail().split("\\_");
