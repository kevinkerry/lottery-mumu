...
public class Hljkl10SS1 extends Hljkl10X{
	...
	public String caculatePrizeLevel(String betcode, String wincode,int oneAmount) {
		...
		for(String code:betcode.split("\\^")) {
			if(code.equals(wincode.substring(0, 2))) {
				sb.append(LotteryDrawPrizeAwarder.HLJK10_S1.value).append(",");
			}
		}
		if (sb.length() > 0) {
	...
	public List<SplitedLot> splitByType(String betcode, int lotmulti,
		...
		List<SplitedLot> zhumaList = transformSingle5(betcode,lotmulti,PlayType.hljkl10_ss1);
