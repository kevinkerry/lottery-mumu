...
public class Hljkl10SR4 extends Hljkl10X{
	...
	public String caculatePrizeLevel(String betcode, String wincode,int oneAmount) {
		...
		for(String code:betcode.split("\\^")) {
			if(totalHits(code, wincode)==4) {
				sb.append(LotteryDrawPrizeAwarder.HLJK10_R4.value).append(",");
			}
		}
		if (sb.length() > 0) {
	...
	public List<SplitedLot> splitByType(String betcode, int lotmulti,
		...
		List<SplitedLot> zhumaList = transformSingle5(betcode,lotmulti,PlayType.hljkl10_sr4);
