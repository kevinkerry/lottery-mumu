...
public class Hljkl10SQ3 extends Hljkl10X{
	...
	public String caculatePrizeLevel(String betcode, String wincode,int oneAmount) {
		...
		for (String code : betcode.split("\\^")) {
			...
			if (codes[0].equals(wincode.substring(0, 2))
				...
				sb.append(LotteryDrawPrizeAwarder.HLJK10_Q3.value).append(",");
			}
		}
		if (sb.length() > 0) {
	...
	public List<SplitedLot> splitByType(String betcode, int lotmulti,
		...
		List<SplitedLot> zhumaList = transformSingle5(betcode,lotmulti,PlayType.hljkl10_sq3);
