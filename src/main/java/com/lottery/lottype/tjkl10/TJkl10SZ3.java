...
public class TJkl10SZ3 extends TJkl10X{
	...
	public String caculatePrizeLevel(String betcode, String wincode,int oneAmount) {
		...
		for (String code : betcode.split("\\^")) {
			if (totalHits(code, wincode.substring(0, 8)) == 3) {
				sb.append(LotteryDrawPrizeAwarder.TJK10_Z3.value).append(",");
			}
		}
		if (sb.length() > 0) {
	...
	public List<SplitedLot> splitByType(String betcode, int lotmulti,
		...
		List<SplitedLot> zhumaList = transformSingle5(betcode,lotmulti,PlayType.tjkl10_sz3);
