...
public class CommonTicketSendWorker extends AbstractTicketSendWorker {
	...
	public void executeSend(TicketBatch ticketBatch, List<Ticket> ticketList, LotteryType lotteryType) throws Exception {
		...
		if (successfulCount == totalCount) {
			logger.info("批次{}全部送票成功", ticketBatch.getId());
			...
			if (reallotCount > 0) {
				...
				if (reallotCount == totalCount - successfulCount) {
					// 直接抛出异常重分当前批次
					logger.error("批次({})送票({})未全部成功,重分终端,{}/{},重分数量:{}", new Object[] { ticketBatch.getId(), terminalType.getName(), successfulCount, totalCount, reallotCount });
					String exceptionMsg=String.format("彩种%s,批次(%s),送票未全部成功", lotteryType,ticketBatch.getId());
					...
					for (Ticket ticket : needToAllotTicketList) {
						...
						try {
							...
							allotWorker.execute(ticket);
							String msg = String.format("票(%s)对方票号(%s)送票失败需要重分, 已自动做拆分成单张批次重新分票处理", ticket.getId(), externalId);
							logger.error(msg);
						} catch (Exception e) {
	...
	protected boolean beforeHandler(TicketBatch ticketBatch, LotteryType lotteryType) throws Exception{
		...
		TerminalConfig  terminalConfig=venderConfigService.getTerminalConfig(lotteryType.value, terminalId, ticketBatch.getPlayType());
