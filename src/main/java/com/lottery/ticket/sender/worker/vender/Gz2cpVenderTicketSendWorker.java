...
import com.lottery.ticket.vender.impl.gz2cp.Gz2cpDes;
import com.lottery.ticket.vender.impl.gz2cp.Gz2cpLotteryDef;
...
@Service
public class Gz2cpVenderTicketSendWorker extends AbstractVenderTicketSendWorker {
	...
	public String getElement(List<Ticket> tickets, String lotteryNo, String phase, IVenderConfig gzcpConfig, LotteryType lotteryType, IVenderConverter gzcpConverter) throws Exception {
		...
		for (Ticket ticket : tickets) {
            ...
            bodyAttr.put("lotterySaleId", Gz2cpLotteryDef.playTypeMap.get(Integer.parseInt(ticket.getContent().split("\\-")[0])));
		...
		String des = Gz2cpDes.des3EncodeCBC(gzcpConfig.getKey(), "<?xml version=\"1.0\" encoding=\"utf-8\"?>" + xml.getBodyElement().asXML());
