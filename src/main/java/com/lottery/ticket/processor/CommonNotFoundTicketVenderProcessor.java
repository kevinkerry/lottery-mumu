...
public class CommonNotFoundTicketVenderProcessor extends AbstractTicketVenderProcessor {
	...
	protected void execute(Ticket ticket, TicketVender ticketVender) {
		...
		if (calendar.getTime().after(ticket.getTerminateTime())) {
			// 过期通知不处理，发送报警邮件转人工处理
			String warn = String.format("收到彩种(%s)第(%s)期票(%s)的出票商未接收票信息, 但已经超过出票截止超时回收时间, 对通知不做任何处理, 转人工处理, orderId=%s, 收到通知时间date=[%s]", lotteryType, ticket.getPhase(), ticket.getId(), ticket.getOrderId(), CoreDateUtils.formatDateTime(date));
			logger.error(warn);
		} else {
			// 获取分票器
			ITicketAllotWorker allotWorker = getAllotExecutor(lotteryType);
			if (allotWorker == null) {
				String allotWarn = String.format("出票商未接收票(%s)的投注信息, 尝试拆分成单张批次重新分票时未找到彩种(%s)的分票器", ticket.getId(), lotteryType);
			...
			try {
				...
				logger.error("票({})重复终端失败", ticket.getId(), e);
			}
		}
	}
