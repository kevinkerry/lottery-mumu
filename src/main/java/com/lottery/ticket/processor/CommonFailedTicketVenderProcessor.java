...
public class CommonFailedTicketVenderProcessor extends AbstractTicketVenderProcessor {
	...
	protected void execute(Ticket ticket, TicketVender ticketVender) {
		...
		logger.error("票({})出票失败,对方原始返回码为:{},是否通知:{},原始返回字符串:{}",ticket.getId(),ticketVender.getStatusCode(),ticketVender.isTicketNotify(),ticketVender.getResponseMessage());
			...
			if (this.checkAllotWithFailureCheck(ticket, venderFailureType, checkFailureType)) {
				try {
					...
					if (newTerminalId != null) {
						// 分票成功
						logger.error("票({})在终端({})出票失败({}), 尝试重新选择其他可用终端分票成功, 新的终端号为: {}", new Object[] { ticket.getId(), oldTerminalId, checkFailureType, newTerminalId });
					} else {
