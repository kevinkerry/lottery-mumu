...
@Service
public class Gz2cpVenderTicketCheckWorker extends AbstractVenderTicketCheckWorker {
	...
	public List<TicketVender> process(List<Ticket> ticketList, IVenderConfig venderConfig,IVenderConverter venderConverter) throws Exception {
		...
		try {
			...
			logger.error("广州2发送:{}查票接口返回异常", messageStr, e);
			return venderList;
		} 
			    ...
			    if(desContent.length()>2){
					...
					if ("0".equals(result)) {
				    	...
				    	for (int i = 0, len = mapLists.size(); i < len; i++) {
						  ...
						  TicketVender ticketVender = createTicketVender(terminalId, TerminalType.T_GZT);
			    	...
			    	logger.error("广州2查票处理结果异常{}",desContent);
			    }
				
		} catch (Exception e) {
			logger.error("广州2查票处理结果异常", e);
		}
	}
	...
	@Override
	protected void init() {
		venderCheckerWorkerMap.put(TerminalType.T_GZT, this);
