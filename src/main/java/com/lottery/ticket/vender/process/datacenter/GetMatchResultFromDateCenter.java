...
public class GetMatchResultFromDateCenter {
	...
	/*
	* 响应：
{"errorCode":"0","message":[{"final_score":"0:1","state":1,"saiguo":"0","spStr":"5.590917","phase":"160401","match_num":"37","lottery_type":"5006"}]}
返回状态  state 0 结果没有采集完成 1 返回成功   2 改期 或 取消
	* */
	public void updateDcSFGGResult(String phase, Integer matchNum) throws Exception{
		...
		String resultStr = HTTPUtil.sendPostMsg("http://filter.dajiang365.com/interphp/bjdc/sf.php", param);
		...
		for (int i = 0; i < messages.size(); i++){
			...
			if(state == 2){
				...
				DcRace dr = dcRaceService.getByPhase(phase, matchNum, DcType.sfgg.value());
				if(dr == null){
					continue;
				}
				if(dr.getPrizeStatus().intValue() != PrizeStatus.result_null.value && dr.getPrizeStatus().intValue() != PrizeStatus.result_set.value 
						&& dr.getPrizeStatus().intValue() != PrizeStatus.pending.value){
					continue;
				}
...
//---------------------------------------------------------------------
	...
	public static void main(String[] args) throws Exception {
		String result = HTTPUtil.sendPostMsg("http://filter.dajiang365.com/interphp/bjdc/sf.php", "phase="+160403+"&match_num="+5);
