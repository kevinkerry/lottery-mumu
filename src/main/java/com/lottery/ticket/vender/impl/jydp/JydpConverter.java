 ...
 */
@Service
public class JydpConverter extends AbstractVenderConverter {
    @Override
    protected Map<LotteryType, String> getLotteryTypeMap() {
        return JydpLotteryDef.lotteryTypeMap;
    }
    ...
    public String convertContent(Ticket ticket) {
		...
		if(JydpLotteryDef.playTypeToAdvanceConverterMap.containsKey(playType)){
			converter= JydpLotteryDef.playTypeToAdvanceConverterMap.get(playType);
		}
		if(converter!=null){
    ...
    public LotteryType getLotteryMap(String lotteryId) {
        return JydpLotteryDef.toLotteryTypeMap.get(lotteryId);
    }

    @Override
    protected Map<Integer, String> getPlayTypeMap() {
        return JydpLotteryDef.playTypeMap;
    }

    @Override
    protected Map<LotteryType, IPhaseConverter> getPhaseConverterMap() {
        return JydpLotteryDef.phaseConverterMap;
    }
    ...
    @Override
    protected void init() {
        venderConverterBinder.put(TerminalType.T_JYDP, this);
    }
    
    
//    public static void main(String[] args) {
//		String content="300412001-20151111301(2)|20151111302(1)^";
//		int lotteryType=3004;
//		String sp="周五301,2(143.5):1.700//周五303,1(210.5):1.750";
//		String []contents=content.split("-")[1].split("\\|");
//		StringBuilder stringBuilder=new StringBuilder();
//		if(lotteryType==LotteryType.JCZQ_SPF.getValue()||lotteryType==LotteryType.JCZQ_SPF_WRQ.getValue()
//		||lotteryType==LotteryType.JCZQ_JQS.getValue()||lotteryType==LotteryType.JCZQ_BQC.getValue()){
//			for(int i=0;i<contents.length;i++){
//				stringBuilder.append(contents[i].split("\\(")[0]).append("(");
//				stringBuilder.append(sp.split("\\//")[i].split("\\,")[1].replace("_", "").replace(":","_").replace("/",",")).append(")");
...
//			for(int i=0;i<contents.length;i++){
//				int lottery=Integer.parseInt(contents[i].split("\\*")[1].split("\\(")[0]);
//				if(lottery==LotteryType.JCZQ_SPF.getValue()||lottery==LotteryType.JCZQ_SPF_WRQ.getValue()
//				||lottery==LotteryType.JCZQ_JQS.getValue()||lottery==LotteryType.JCZQ_BQC.getValue()){
//					stringBuilder.append(contents[i].split("\\(")[0]);
//					stringBuilder.append("(");
//					stringBuilder.append(sp.split("\\//")[i].split("\\,")[1].replace("_", "").replace("R", "").replace("B", "")
