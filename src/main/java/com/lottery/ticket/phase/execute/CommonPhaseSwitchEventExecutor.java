...
public class CommonPhaseSwitchEventExecutor extends AbstractPhaseEventExecutor {
	...
	protected boolean execute(IScheduledTask itask) {
		...
		// 关闭当前彩期
		{
			boolean flag = true;
			try {
				phaseService.phaseSwitch(PhaseStatus.close.value, YesNoStatus.no.value, YesNoStatus.no.value, currentPhase.getId());
			} catch (Exception e) {
		...
		// 切换下一彩期
		{
			...
			try {
				...
				if (nextPhaseList != null && nextPhaseList.size() > 0) {
					...
					phaseService.phaseSwitch(PhaseStatus.open.value, YesNoStatus.yes.value, YesNoStatus.yes.value, id);
					if (LotteryType.getZc().contains(lotteryType)) {
						...
						phaseService.updateTerminalStatus(TerminalStatus.enable.value, id);
					}
			...
			logger.info("({})新的当前彩期({})", lotteryType, nextPhase.getPhase());
		}
	...
	protected IScheduledTask generateScheduledTaskForNextEvent(IScheduledTask currentTask) {
		...
		logger.info("彩期切换，传递给下一事件的任务，{}，{}", nextTask.getLotteryType(), nextTask.getCurrentPhaseNo());
		return nextTask;
	}
