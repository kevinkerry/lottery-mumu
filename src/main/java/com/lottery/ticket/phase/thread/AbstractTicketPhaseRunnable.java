...
public abstract class AbstractTicketPhaseRunnable extends AbstractThreadRunnable {
	...
	public long getEndSaleForward() {
		if(lotteryTicketConfigCacheModel!=null){
			try {
				LotteryTicketConfig lotteryTicketConfig=lotteryTicketConfigCacheModel.get(getLotteryType());
				if(lotteryTicketConfig!=null){
					if(lotteryTicketConfig.getEndSaleForward()!=null){
						endSaleForward=lotteryTicketConfig.getEndSaleForward().longValue();
					}
				}
			}catch (Exception e) {
				...
				try{
					...
					for (LotteryType lotteryType : this.getLotteryTypeList()){
						...
						if (orders != null && !orders.isEmpty()) {
							logger.error("竞彩[{}],[{}]期存在未开奖订单[{}]，不能修改状态为已开奖", new Object[]{lotteryType.name,phase, orders.get(0).getId()});
							planFlag = true;
							break;
						} 
					}
