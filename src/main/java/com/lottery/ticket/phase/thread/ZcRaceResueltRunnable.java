...
public class ZcRaceResueltRunnable extends AbstractThreadRunnable {
    protected long interval=180000;
    @Autowired
    private LotteryPhaseService lotteryPhaseService;
    @Autowired
    private ZcRaceService zcRaceService;
    private String url="http://filter.dajiang365.com/interphp/zcrace/";
        ...
        for(int lotteryType: LotteryType.getZcValue()){
            ...
            if (lotteryPhaseList!=null){
                for (LotteryPhase lotteryPhase:lotteryPhaseList){
                    ...
                    String responseStr= HTTPUtil.sendPostMsg(url,param);
                    try{
                        ...
                        if(errorcode.equals("0")){
                            ...
                            if(jsArray!=null){
                                for(int i=0;i<jsArray.size();i++){
                                    ...
                                    int isoutmoney=jsObject.getInt("isoutmoney");
                                    if (isoutmoney==0){
                                        continue;
                                    }
                                    ...
                                    lotteryPhase.setWincode(getWincode(result));
