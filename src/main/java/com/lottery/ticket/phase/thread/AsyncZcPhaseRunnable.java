...
public class AsyncZcPhaseRunnable extends AbstractThreadRunnable{
	...
	private String url="http://filter.dajiang365.com/interphp/zc/presale.php";
	

	private String matchUrl="http://filter.dajiang365.com/interphp/zc/";
	...
	private   void process( int lotteryType)throws Exception{
		...
		String responseStr= HTTPUtil.sendPostMsg(url,"lottery_type="+lotteryType);
		try{
			...
			if(errorcode.equals("0")){
				...
				if(jsArray!=null){
					for(int i=0;i<jsArray.size();i++){
						...
						String starttime=jsObject.getString("startsaletime");
						String endtime=jsObject.getString("endsaletime");
						this.updatePhase(lotteryType,phase, CoreDateUtils.parseDateTime(starttime),CoreDateUtils.parseDateTime(endtime));
	...
	protected void against(int lotteryType,String phase)throws Exception{
		...
		String responseStr=HTTPUtil.sendPostMsg(matchUrl,"lottery_type="+lotteryType+"&phase="+phase);
