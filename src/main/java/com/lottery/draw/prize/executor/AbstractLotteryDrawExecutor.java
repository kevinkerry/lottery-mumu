...
public abstract class AbstractLotteryDrawExecutor implements ILotteryDrawExecutor{
	...
	public void execute(ILotteryDrawTask lotteryDrawTask) throws Exception {
		...
		logger.info("初始化开奖订单总数");
		int drawOrderTotal=0;
		try {
			drawOrderTotal=initDrawOrderTotal(lotteryDrawTask);
		} catch (Exception e) {
			logger.error("初始化开奖订单总数异常，开奖任务信息：{}", lotteryDrawTask);
			logger.error("初始化开奖订单总数异常", e);
			return;
		}
		if(drawOrderTotal==0){
			try {
				this.afterDrawHandle(lotteryDrawTask);
			} catch (Exception e) {
				logger.error("无开奖订单出错",e);
