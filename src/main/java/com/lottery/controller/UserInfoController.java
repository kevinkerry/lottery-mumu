...
public class UserInfoController {
	...
	public @ResponseBody ResponseData mobileLogin(@RequestParam(value = "mobilephone", required = true) String mobilephone, @RequestParam(value = "password", required = true) String password) {
		...
		try {
			...
			try {
				userInfo = userService.getByPhoneNo(mobilephone);
			} catch (Exception e) {
	...
	@RequestMapping(value = "/uniqueInfo", method = RequestMethod.POST)
	public @ResponseBody ResponseData uniqueInfo(@RequestParam("propertyName") String propertyName, @RequestParam("propertyValue") String propertyValue) {
		ResponseData rd = new ResponseData();
		List<UserInfo> info = null;
		try {
			if ("lincardid".equals(propertyName)) {
				info = userService.getByPropertyName("idcard", propertyValue);
			} else if ("username".equals(propertyName)) {
				info = userService.getByPropertyName("username", propertyValue);
			} else if ("phone".equals(propertyName)) {
				info = userService.getByPropertyName("phoneno", propertyValue);
			} else if ("id".equals(propertyName)) {
				info = userService.getByPropertyName("userno", propertyValue);
			} else if ("alias".equals(propertyName)) {
				info = userService.getByPropertyName("alias", propertyValue);
			}

			if (info != null && info.size() > 0) {
				rd.setErrorCode(ErrorCode.isused.getValue()); // 已被使用
				rd.setValue(info.get(0));
			} else {
				rd.setErrorCode(ErrorCode.Success.getValue());
				rd.setValue(new UserInfo());
			}
		} catch (Exception e) {
			rd.setErrorCode(ErrorCode.system_error.getValue());
			logger.error("通过键值对查询用户信息失败", e);
		}
		return rd;
	}
