...
public class LottypeConfigController {
    ...
    @RequestMapping(value = "/validateConfigList", method = RequestMethod.POST)
    public @ResponseBody ResponseData validateConfigList() {
        ResponseData rd = new ResponseData();
        try {
            rd.setErrorCode(ErrorCode.Success.value);
            rd.setValue(phaseHandler.getValidateConfigList());
        } catch (Exception e) {
            logger.error("查询toltypeconfig所有配置异常", e);
            rd.setErrorCode(ErrorCode.system_error.value);
            rd.setValue(e.getMessage());
