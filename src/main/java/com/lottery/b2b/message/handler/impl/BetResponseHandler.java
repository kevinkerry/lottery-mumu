				...
				try {
					...
					if (total.compareTo(new BigDecimal(amountBlance)) < 0) {
						...
						if (merchantOrder != null) {
							...
							String betOrder = idGeneratorService.getLotteryOrderId();
							betService.bet(userno, orderId, betcode, betOrder, lotteryType, phase, playType, multipl, amountBlance, addition,b2bForward,endtime);
					...
					logger.error("messageid={},error={}",messageid,e.getMessage());
					logger.error("messageid={},orderid={}送票出错", messageid, orderId, e);
					logger.error("messageid={},error={}",messageid,e.getMessage());
					logger.error("messageid="+messageid,e);
