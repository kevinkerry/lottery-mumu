...
public class TicketDAOImpl extends AbstractGenericDAO<Ticket, String> implements TicketDAO {
	...
	public List<Object[]> getIdsByTerminalIdAndStatus(Long terminalId, int status, int max) {
		...
		String sql = "select id,deadline,sendTime from Ticket o where o.status = :status ";
		if(terminalId != null){
		...
		Query query = entityManager.createQuery(sql + " order by deadline asc");
	...
	@Override
	public Long getCountByTerminalIdAndStatus(Long terminalId, int status) {
		String sql = "select count(id) from Ticket o where o.status = :status ";
		if(terminalId != null){
			sql += " and terminalId = :terminalId";
		}
