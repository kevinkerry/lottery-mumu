...
public class PrizeHandler {
		...
		if (order.getBetType() == BetType.hemai.getValue()) {
			...
			if (caselot != null) {
				try {
					...
					if(caselot.getIsExchanged()!=null&&caselot.getIsExchanged()==YesNoStatus.no.value){
	 ...
	 * 检查算奖
	 * */
	public Boolean prizeEndCheck(int lotteryType, String phase) throws Exception {
	...
	public  void prizeCallbak(int lotteryType,String phase)throws Exception{
       boolean flag=prizeEndCheck(lotteryType,phase);
		if (flag){
