...
import com.lottery.retrymodel.prize.error.ModifyRaceTimeCallback;
...
public class PrizeErrorHandler {
     ...
     * @param deal        是否撤单
     */
    public void modifyTime(int lotteryType, String phase, String matchNum, boolean deal) {
        ModifyRaceTimeCallback modifyRaceTimeCallback = new ModifyRaceTimeCallback();
        modifyRaceTimeCallback.setTicketService(ticketService);
        modifyRaceTimeCallback.setName("修改竞彩北单时间");
        modifyRaceTimeCallback.setFlag(deal);
        modifyRaceTimeCallback.setLotteryOrderService(lotteryOrderService);
        modifyRaceTimeCallback.setLotteryType(lotteryType);
        modifyRaceTimeCallback.setMatchNum(matchNum);
        modifyRaceTimeCallback.setPhase(phase);
        modifyRaceTimeCallback.setQueueMessageSendService(queueMessageSendService);
        modifyRaceTimeCallback.setJingcaiService(jingcaiService);
        modifyRaceTimeCallback.setBeidanService(beidanService);
        apiReryTaskExecutor.invokeAsync(modifyRaceTimeCallback);
    }
