...
public class PrizeService {
		...
		while (null != tickets && tickets.size() != 0) {
			for (Ticket ticket : tickets) {
				try {
					...
					logger.error("算奖出错", e);
					throw new LotteryException("算奖出错");
				}
			}
			start = start + 1;
	...
	private void onPrize(Ticket ticket, PrizeResult result) {
		...
		if (result.isBig()) {
			contentMap.put("ticketResultStatus", TicketResultStatus.win_big.value);
		} else {
			contentMap.put("ticketResultStatus", TicketResultStatus.win_little.value);
		}
 ...
 * */
	...
	public List<LotteryPhase> asyncPhase(int lotteryType, int terminalType) {
		...
		return worker.getOnSalePhaseList(lotteryType);
	}
