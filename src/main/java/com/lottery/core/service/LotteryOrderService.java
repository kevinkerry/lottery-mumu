...
public class  LotteryOrderService {
	...
	public  void updateOrderResultStatus(String orderId,int orderResultstatus){
		lotteryOrderDao.updateOrderResultStatus(orderId, orderResultstatus);
	}
	/**
	 * 根据彩种,彩期,状态,开奖状态查询
	 * @param lotteryType 彩种
	...
	public int updateWincode(int lotteryType,String phase,String wincode){
		return  lotteryOrderDao.updateWincode(lotteryType, phase, wincode);
	}

	public List<Object[]> getMonitorData(Integer lotteryType, Date deadline, List<Integer> ts) {
		return lotteryOrderDao.getByLotteryPhaseAndStatusForMonitor(lotteryType, deadline, ts);
	}
	
	public TicketMonitor getMonitorDataDetail(Integer lotteryType, String phase, OrderStatus status){
		...
		List<Object[]> ids = lotteryOrderDao.getByLotteryPhaseAndStatus(lotteryType, phase, status.value, 200);
		if (ids.size()>0){//只查询有值的
			monitor.setIds(ids);
			int size = ids.size();
			if(size == 200){
				monitor.setSize(lotteryOrderDao.getCountByLotteryPhaseAndStatus(lotteryType, phase, status.value));
			}else{
